%% Example_2_Integral_Control.m
%
% Integral Control
%
% Related Slides:         2-54 to 2-56
% Related Simulink Model: Example_2_Integral_Control_Mdl.mdl
%
...
%                         Remove state_command matrix N_x (in the Simulink model)
%                         Modify output matrix C to simulate parametric uncertainty (in the Simulink model)
...
%% Augmentation of the Discrete-Time System
n = size(A);
A_aug = [1 C; zeros(n,1) A];
B_aug = [0; B];

%% Controllability of the Augmented Discrete-Time System
C_aug_ctrb = ctrb(A_aug,B_aug) % controllability matrix
rank(C_aug_ctrb)               % rank of the controllability matrix

%% Integral Control Design
n_aug = size(A_aug);
lambda_aug_tilde = [0.8; exp(lambda_tilde_c*h)]
K_aug = place(A_aug,B_aug,lambda_aug_tilde);
K_I = -K_aug(1)                % Caution! Sign defined differently in MATLAB!
K = -K_aug(2:n_aug)            % Caution! Sign defined differently in MATLAB!

%% State Command Matrix Design
[n m] = size(B);
[p n] = size(C);
